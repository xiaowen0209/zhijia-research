...
const GLOSSARY_DATA = [
  ...
  // 坐标系与标定
  { term: "坐标系", full: "Coordinate Systems", desc: "自动驾驶涉及多种坐标系：世界坐标系(WGS84/GPS)、车辆坐标系(ISO 8855，以车中心为原点)、传感器坐标系(各摄像头/LiDAR/雷达相对安装位置)。坐标系转换是感知融合的基础。" },
  { term: "内外参标定", full: "Intrinsic / Extrinsic Calibration", desc: "内参标定确定传感器自身光学/物理参数(焦距、畸变、畸变中心)；外参标定确定多传感器之间的相对位姿关系(平移+旋转)。标定精度直接影响感知融合效果和测距精度。" },
  { term: "时间同步", full: "Time Synchronization", desc: "确保多传感器(GNSS/IMU/摄像头/LiDAR)数据时间戳对齐的技术。通常采用PTP或gPTP协议，同步精度需达到微秒级。时间不同步会导致运动目标位置偏移，是感知融合的前提条件。" },

  // 常见行为问题
  { term: "偏航", full: "Lane Departure / Yaw Deviation", desc: "车辆行驶轨迹偏离车道中心线。可能原因：定位误差、感知车道线丢失、控制参数失配或横风干扰。严重时导致压线或驶出车道，是LKA/AEB的重要触发条件。" },
  { term: "限速响应", full: "Speed Limit Response", desc: "自动驾驶系统对道路限速标识的识别和响应能力。包含限速牌识别(TSR)、地图限速数据融合、弯道/匝道限速预判。要求平滑减速而非急刹，过早减速或过晚响应均影响评价。" },
  { term: "过度避让", full: "Over-cautious Avoidance / Overly Conservative", desc: "系统对潜在风险反应过度保守，不必要的减速或绕行。虽保障了安全性，但牺牲了通行效率和驾乘体验，是L4系统'过度安全'的典型工程取舍问题。" },
  { term: "变道决策", full: "Lane Change Decision", desc: "自动驾驶系统判断是否、何时、如何变道的决策过程。需综合评估目标车道空间、后车速度、可接受间隙、变道收益(更快/更优路线)。决策犹豫或激进均会导致接管。" },
  { term: "压线", full: "Lane Encroachment / Line Crossing", desc: "车辆轮胎或车身越过车道边界线。原因：定位偏差、感知车道线丢失、规划轨迹过切、控制超调。高频压线严重影响安全评价和用户安心感。" },
  { term: "选道犹豫", full: "Lane Selection Indecision", desc: "系统在多车道场景下无法快速确定最优车道，表现为左右反复评估、长时间不决策。导致通行效率下降、后车催促进而触发效率接管。" },

  // 人机交互与评价
  { term: "人机共驾", full: "Human-Machine Co-driving", desc: "驾驶员与自动驾驶系统共享车辆控制权的驾驶模式。L2-L3阶段的典型形态，核心挑战在于接管时序、权限切换和驾驶员状态监测，目标是实现安全顺畅的人机协作。" },
  { term: "评价四维度", full: "Safety / Efficiency / Trust / Comfort", desc: "自动驾驶系统评价的四大维度：安全性(无事故/少接管)、效率(通行时间/速度)、安心感(用户对系统的信任程度)、舒适感(加减速平顺/转向柔和)。四者需要平衡，不可偏废。" },
  { term: "触发机制", full: "Trigger / Activation Mechanism", desc: "自动驾驶功能的激活条件和触发方式。包括ODD满足确认、驾驶员一键激活、系统自检通过等。触发机制需清晰、可信、防误触，是HMI设计的关键要素。" },
  { term: "接管分级", full: "Intervention Classification", desc: "对接管事件按原因的系统分类。安全接管(系统未识别风险/违反交规)、效率接管(策略不当/选道不佳)、体验接管(过度保守/不舒适)。分级统计驱动针对性算法优化。" },

  // 开发工具链
  { term: "Foxglove", full: "Foxglove Studio", desc: "现代机器人数据可视化与调试平台，支持MCAP/ROS Bag等格式的数据回放。提供Web端面板布局、3D场景渲染、时序图表、消息检查器等功能，是自动驾驶数据分析的主流工具。" },
  { term: "评测数据集", full: "Benchmark Dataset Construction", desc: "构建标准化评测数据集的流程：多源数据筛选清洗→标注分类→场景标准化固化→绑定量化指标→持续扩充。高质量评测数据集是客观衡量自动驾驶性能的标尺。" },
  { term: "内场测试", full: "Indoor / Lab Testing (SIL/HIL)", desc: "在实验室可控环境中进行的测试。SIL(软件在环)纯软件仿真，HIL(硬件在环)接入真实硬件。优势是可复现、成本低、可大规模回归；局限是无法覆盖真实世界的复杂性。" },
  { term: "外场测试", full: "On-road / Field Testing", desc: "在真实公共道路上进行的实车路测。采集真实道路数据、验证系统在复杂动态环境中的表现。外场数据是数据闭环的起点，也是内场评测场景的主要来源。" },

  // 行业标准
  { term: "ISO 26262", full: "ISO 26262 — Functional Safety", desc: "道路车辆功能安全国际标准。定义ASIL(A-D)四个安全完整性等级，规范从概念到报废全生命周期的功能安全管理。自动驾驶系统核心控制器通常需达到ASIL-D最高等级。" },
  { term: "ISO 21448", full: "ISO 21448 — SOTIF", desc: "预期功能安全标准。关注系统在无硬件故障情况下因性能局限(传感器在恶劣天气下降、AI算法误判罕见场景)导致的潜在风险。是AI驱动的自动驾驶系统特有的安全标准。" },
  { term: "UN R157", full: "UN Regulation No.157 — ALKS", desc: "联合国自动车道保持系统法规。全球首个L3级自动驾驶国际法规，规定60km/h以下高速公路ALKS的技术要求、测试方法和审核准则，于2021年生效。" },
  { term: "GB/T 40429", full: "GB/T 40429-2021", desc: "中国自动驾驶数据记录系统国家标准。规范L3+自动驾驶汽车的DSSAD(数据存储系统)技术要求，包括碰撞前数据记录范围、存储时长、数据提取接口等，为事故责任判定提供依据。" }
];
